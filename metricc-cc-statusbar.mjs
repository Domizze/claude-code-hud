...
const SECTION_DEFAULTS = {
  ...
  "Session": false, "Changes": false, "Directory": false, "Repo": false, "Cost": false,
...
// ── Git repo / worktree ───────────────────────────────────────────────────────
// ponytail: one git process per render (~10ms). Cache by cwd if it ever drags.
function getRepo(dir) {
  if (!dir) return null;
  try {
    const out = execSync(
      "git rev-parse --show-toplevel --git-dir --git-common-dir --abbrev-ref HEAD --short=7 HEAD",
      { cwd: dir, encoding: "utf-8", timeout: 2000, stdio: ["ignore", "pipe", "ignore"] },
    ).trim().split(/\r?\n/).map((l) => l.trim());
    if (out.length < 5) return null;
    const [top, gitDir, commonDir, branch, sha] = out;
    return {
      name: basename(top),
      ref: branch === "HEAD" ? sha : branch,
      // a linked worktree's .git dir sits under the main repo's common dir
      isWorktree: resolve(dir, gitDir) !== resolve(dir, commonDir),
    };
  } catch {
    return null;
  }
...
function render(usage, transcript, contextPct, modelId, version, latestVersion, cost, stdinData, config) {
  ...
  // Repo (name, branch, worktree marker)
  if (show("Repo")) {
    const repo = getRepo(stdinData?.workspace?.current_dir);
    let repoValue;
    if (repo) {
      const wt = repo.isWorktree ? `${c.yellow}worktree ${c.reset}` : "";
      repoValue = `${c.slate600}${repo.name}${c.reset} ${c.slate800}|${c.reset} ${wt}${c.slate600}${repo.ref}${c.reset}`;
    } else {
      repoValue = `${c.slate600}-${c.reset}`;
    }
    columns.push({ label: `${c.slate800bold}Repo:${c.reset}`, value: repoValue });
  }
